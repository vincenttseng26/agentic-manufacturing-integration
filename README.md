...
### Sample data (no GPU needed)

Rollouts are produced on a GPU by the sim layer, but the DB / analytics / Power BI steps aren't.
So you can reproduce them without the simulator, the repo ships a real result set at
`data/sample_sweep.jsonl` — **20 training checkpoints × 300 matched-seed rollouts (6,000 rows)**,
the exact data behind the checkpoint experiment above.

```bash
docker compose up -d
python orchestration/load_results.py data/sample_sweep.jsonl --batch-id 1
python analysis/export_powerbi.py --out-dir data   # KPI + SPC CSVs for Power BI
```
