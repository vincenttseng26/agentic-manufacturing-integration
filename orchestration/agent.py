...
MODEL = "gemini-3.1-flash-lite"  # 免費層配額按模型分開算；2.5-flash 每日 20 次很快用完，lite 額度較寬
MAX_STEPS = 6

SYSTEM_INSTRUCTION = """你是「智慧製造工作站」的監控助理。可用工具（透過 MCP）：
- get_kpi：整體成功率 / 失敗階段 Pareto / 各顏色方塊失敗率；
  可用 model_tag 參數只統計單一 checkpoint（例 model_tag="epoch_300"），留空＝全部資料。
...
回答一般 KPI 問題（含單一 checkpoint 的 KPI）時優先用 get_kpi；要更細的統計才寫 SELECT 給 query_db。
用繁體中文簡潔回答，並點出重點發現。"""
...
async def _converse(client, session, config, contents, model) -> str:
    ...
    for _ in range(MAX_STEPS):
        ...
        if not calls:
            return resp.text
        for call in calls:
            print(f"  [呼叫工具] {call.name} {dict(call.args or {})}")
...
async def chat(model: str = MODEL):
    ...
    print("請用自然語言查詢方塊分類任務執行狀態（輸入 exit / quit 離開）")
    print("可以詢問：成功率、失敗階段、cycle time…，或「跑 10 個任務」(需 GPU)。\n")
    async with stdio_client(_server_params()) as (read, write):
        async with ClientSession(read, write) as session:
            await session.initialize()
            listed = await session.list_tools()
            ...
            contents: list = []
            while True:
                q = (await loop.run_in_executor(None, input, "問題 > ")).strip()
                ...
                try:
                    ...
                    # 429 配額用完等 API 錯誤：印一行提示就好，session 與上下文保留，別整個炸掉
                    print(f"\n(Gemini API 錯誤 {e.code}: {e.status}，稍後再問一次即可)\n")
                    continue
                print(f"\nAgent 回覆 > {answer}\n")
