...
用法（從專案根目錄）：
    # 穩定監控 200/300/600，每小時各 20 個（需 GPU，長駐；Ctrl+C 停）
    ~/env_agent/bin/python -m orchestration.scheduler --epochs 200,300,600 --interval-minutes 60 --batch-size 20
    # demo：每 2 分鐘一輪、各 5 個
    ~/env_agent/bin/python -m orchestration.scheduler --epochs 200,300,600 --interval-minutes 2 --batch-size 5
    # 只跑一輪就結束
    ~/env_agent/bin/python -m orchestration.scheduler --epochs 200,300,600 --once
    # 空跑測排程（不跑 sim）
    ~/env_agent/bin/python -m orchestration.scheduler --epochs 200,300,600 --dry-run --interval-minutes 0.1
"""
import argparse
import pathlib
import sys
...
def main():
    ...
    p.add_argument("--epochs", type=str, default=DEFAULT_EPOCHS, help="逗號分隔，如 200,300,600")
